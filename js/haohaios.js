...
newDiv.setAttribute('name','HHS_FORMBUY');
  //生成层内内容  
  newDiv.innerHTML = '<a href="javascript:cancel_div()" id="sku-quit"></a><div id="sku-head"><img id="sku-image" class="image" src="'+goods_thumb+'"><div id="sku-detail"><div class="sku-name"><span class="goodsName" id="goodsName">'+goods_name+'</span><div class="sku-price-depends">￥'+goods_price+'</div></div><div><span id="sku-msg">' + select_spe + '</span></div></div></div>';
...
specDiv.setAttribute('class','sku-info');
  ...
  newDiv.innerHTML += '<div class="sku-amount"><div class="sku-text"><a>购买数量</a><div class="nbox"><i class="fa fa-minus" onclick="jian();"></i><input id="number" name="number" value="1" class="num" type="text" maxlength="5"><i class="fa fa-plus" onclick="jia();"></i></div></div></div>';
...
function cancel_div() 
{
 ...
 /*       $('body').on('click', '.sku-buy-amount-increase', function(event) {
...
*/
   /*     $('body').on('click', '.sku-buy-amount-reduce', function(event) {
          event.preventDefault();
          // Act on the event 
          var inum = $("#number").val();
          if(inum<=1)
...
*/
        $('body').on('change', '.sku-info input', function(event) {
          ...
          $.each($(".sku-info input:checked"), function(index, val) {
              ...
              new_price += attr_price;
          });
          new_price = parseFloat(goods_price) + new_price;
          $('.sku-price-depends').text('￥'+new_price.toFixed(2));
        });
