...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    {
        $where = '  og.goods_id = ' . $goods->goods_id . ' and oi.order_status=1 and oi.pay_status=2 ';
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    {
        $has_express   = $db->getOne('select `goods_id` from '.$hhs->table('goods_express').' WHERE goods_id = ' . $goods_id);
        if (! empty($has_express)) {# 存在模板运费的
            $shipping_list = array();
            $point_list = array();
        }
    }
...
elseif ($_REQUEST['step'] == 'json_done')
{
    ...
    { 
        //判断是否是自己的
        $sql="select count(*) from ".$hhs->table('order_info') ." where pay_status > 1 AND team_sign=".$_SESSION['team_sign']." and user_id=".$_SESSION['user_id'];
        $temp=$db->getOne($sql);
        if($temp>0){
            $result['error']=1;
			if($_SESSION['is_luck'])
			{
				$result['url']="luck_msg.php?team_sign=".$_SESSION['team_sign']."&uid=".$uid;
			}
			else
			{
