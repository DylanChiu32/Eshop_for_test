...
$size  = !empty($_REQUEST['page_size']) ? intval($_REQUEST['page_size']) : 20;

$order = '';
$site_id  = $_REQUEST['site_id'];

$count = get_mall_goods_count($cat_id,$site_id);
...
function get_mall_goods($cat_id, $size, $page, $sort, $order,$site_id){
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $arr[$idx]['market_price']  = app_price_format($row['market_price']);
        $arr[$idx]['shop_price']    = app_price_format($row['shop_price']);
