...
function get_comment_list($suppliers_id)
{
    ...
    $arr = array();
    $sql  = "SELECT c.*,s.suppliers_id FROM " .$GLOBALS['hhs']->table('comment'). " as c left join ".$GLOBALS['hhs']->table('goods')." as s on c.id_value = s.goods_id WHERE c.parent_id = 0 and comment_type = 0 and  s.suppliers_id = ".$suppliers_id." order by c.add_time desc";
	$res  = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
