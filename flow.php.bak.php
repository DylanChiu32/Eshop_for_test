...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    //判断商品是否限购
    $limit_row=$db->getRow("select limit_buy_one,limit_buy_bumber from ".$hhs->table('goods')." where goods_id =".$goods->goods_id." and  is_delete=0  ");
...
elseif ($_REQUEST['step'] == 'checkout')
{
  ...
  /* 获得用户所有的收货人信息 */
    ...
    $_SESSION['goods_suppliers_id'] = $cart_goods[0]['suppliers_id'];
    /* 取得配送列表 */
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
    $shipping_list     = available_shipping_list($region);
...
//var_dump($order);exit();
    ...
    $sql = "SELECT bonus_allowed FROM ".$hhs->table("goods")." WHERE goods_id = " . $goods_id;
    $bonus_allowed = $db->getOne($sql);
    ...
    if($allow_fenxiao)
    {
        ...
        if($users['uid_1'] > 0 && $rates['rate_1'] > 0){
            ...
            if($money > 0.00)
            {
                ...
                $weixin->send_wxmsg($users['openid_1'], '' , '' , $description );
            }
        }
        if($users['uid_2'] > 0 && $rates['rate_2'] > 0){
            ...
            if($money > 0.00){
                ...
                $description = '您分享的商品经过长途跋涉 终于有回报了' . $money;
                $weixin->send_wxmsg($users['openid_2'], '' , '' , $description );
            }
        }
        if($users['uid_3'] > 0 && $rates['rate_3'] > 0){
            ...
            if($money > 0.00){
                ...
                $description = '您分享的商品经过长途跋涉 终于有回报了' . $money;
                $weixin->send_wxmsg($users['openid_3'], '' , '' , $description );
            }
        }
    }
