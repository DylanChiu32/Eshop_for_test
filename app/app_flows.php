...
if($action =='add_to_cart')
{
   ...
   // $_POST['goods'] = json_str_iconv($_POST['goods']);
	...
	$goods['number'] = $_POST['number'];
	$goods['rec_type'] = $_POST['rec_type'];
	$goods['type'] =  $_POST['type'];
	$goods['team_sign'] =  $_POST['team_sign'];
...
elseif($action =='cart')
{
	...
	foreach($cart_goods as $idx=>$v)
	{
		...
		foreach($goods_list as $ids=>$s)
		{
			...
			{
				$s['goods_list'][$id]['goods_img'] = $redirect_uri.$dd['goods_img'];
			}
...
elseif ($action == 'change_bonus')
{
    ...
    else
    {
        ...
        if ((!empty($bonus) && $bonus['user_id'] == $user_id && $bonus['suppliers_id'] == $suppliers_id) || $_GET['bonus'] == 0)
        {
